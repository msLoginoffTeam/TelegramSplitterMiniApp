...
- Форма траты допускает неполное распределение: при положительной нераспределённой сумме пользователь подтверждает сохранение черновика. Backend сам выводит `IsDraft`; черновик не участвует в balances/transfers до распределения всей суммы. Лейбл и раскрываемое пояснение не маскируют этот факт.
- Платёж, связанный с тратой, может превышать текущую долю. Доля будет погашена, а переплата останется кредитом отправителя; API отдаёт агрегированные `PaidAmount` и `OverpaymentAmount` для компактного отображения в деталях траты.
- Удаление траты и платежа — soft delete на backend. Перед удалением траты Mini App перечисляет все связанные платежи; после подтверждения они исчезают из рабочих списков, balances и transfers, но остаются в audit history и БД для будущего восстановления администратором.
...
- `AmountInput` — shared UI-компонент сумм: поддерживает выражения с `+`/`-` (например, `1200+350-50`), показывает результат и подставляет точное число по нажатию. Его используют сумма траты, доли и платежи. В форме траты у каждой выбранной доли есть компактное действие `+/- сумма` для применения всего нераспределённого остатка к этой доле.
- Навигация использует встроенный React Router `ScrollRestoration`; при возврате по истории восстанавливается позиция страницы. Dashboard группы отдельно сохраняет в `sessionStorage` раскрытие трёх сводок (баланс, траты, платежи) на время текущей сессии Mini App.
- Корневой route имеет React Router `errorElement`: ошибки рендера страницы не раскрывают пользователю технический stack trace, а показывают локальный экран с обновлением приложения и возвратом к группам.
